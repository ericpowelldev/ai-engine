...
- `wire.sh` — the wiring refresh, the mechanical half of `/setup` (also run by `/orient` and `/pull`): writes the always-on import block, installs commands, generates the rule-type skills from `registries/rule-types.md`, syncs module wiring, registers module hooks, maintains the install manifest. Idempotent.
- `pull.sh` — the mechanical half of `/pull`: pulls the baseline repo and every module repo (ff-only; skips repos with uncommitted changes or no remote), then runs `wire.sh` so pulled wiring and rules take effect.
