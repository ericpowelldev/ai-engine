...
1. Run `bash {{AI_DIR}}/hooks/pull.sh` — it pulls the baseline repo and each module repo (ff-only, skipping any with uncommitted changes or no remote), then re-runs `setup.sh` so pulled wiring, rules, and hooks take effect.
