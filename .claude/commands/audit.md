...
- **No argument** → audit the baseline (every committed file).
...
- **`all`** → the baseline plus every module.

This is a full-content audit: **read every in-scope file completely** — for the baseline: `CLAUDE.md`, `README.md`, `SETUP.md`, every folder README, every rules file, every hook script, everything in `.claude/`; for a module: every module file. Skimming disqualifies the audit — a file is only audited once it's been read to the last line. Sections 1, 2, and 6 apply to whatever is in scope; 3–5 are baseline concerns (include them when the baseline is in scope). **When a module is in scope and has an `audit.md` at its root, also run its checks** — org-specific audit direction lives there. Report findings; propose fixes but apply nothing without approval.

## 1. Reference integrity (the core pass)

Every pointer in every file must resolve. Check, in each file:

- **File paths** mentioned in prose (`rules/rules-coding.md`, `hooks/setup.sh`, `~/.claude/...`) — the target exists.
...
- No specific module names in committed plumbing or docs.
- Gitignore split intact: `personal/`, `modules/`, `scratchpad/` contents ignored, READMEs kept.
...
- Structure: every module has a README with a concrete activation scope, no `personal/` or nested `modules/` inside; every root folder has its README.
