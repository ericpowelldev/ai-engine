...
1. Run `bash {{AI_DIR}}/scripts/push.sh "$ARGUMENTS"` (omit the argument if none was given — the script defaults to `sync: <date>`). Per module it: stages everything and commits when the tree is dirty, pushes any unpushed commits (setting the upstream on a first push), and skips modules that are clean and synced or have no remote. Current branch only, never force, and **never the baseline repo** — engine commits are hand-written by the user.
