...
|---|---|
| `/orient [module]` | Orient a session: runs the wiring refresh first (self-healing), then loads the always-on modules and the scoped module (fuzzy-matched argument or automatic scope detection), following the module's own `orient.md` when it has one |
...
| `/rule-lookup [name\|type\|keyword]` | Look up a rule by exact name, registered type, or keyword and show its current wording from the module rule files; no argument prints the rule inventory |
